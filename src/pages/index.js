...
const getInfoFromServer = new Api({
...
const delCardPopup = new PopupDelCard('#del-card-popup', 
   (card, cardId) => {
      getInfoFromServer.deleteCard(cardId)
      .then(() =>{
         card.removeCard();
...
const createCard = (argument) => {
   const card = new Card({
      ...
      handelLike: (likeCounter) => {
         getInfoFromServer.setLike(argument._id)
         .then(card => {
      ...
      handleDeleteLike: (likeCounter) => {
         getInfoFromServer.deleteLike(argument._id)
         .then(card => {
            likeCounter.textContent = card.likes.length;
         })
...
const newCardPopup = new PopupWithForm('#gallery-popup',
   (inputsValues) => {
      const promise = new Promise(resolve => {
         ...
         getInfoFromServer.addPicture({
         pictureName: inputsValues['img-name'],
         link: inputsValues['img-location']
      })
      .then(resolve)
      })
      ...
      .catch(err => console.log(`Ответ новой картинки не пришел ${err}`))
   }
);
...
const profilePopup = new PopupWithForm('#profile-popup',
   (inputsValues) => {
      const promise = new Promise(resolve => {
         ...
         getInfoFromServer.changeProfile({
            userName: inputsValues['user-name'], 
      ...
      .catch(err => console.log(`Ответ редактирования профиля не пришел ${err}`))
   }
);
...
const avatarPopup = new PopupWithForm('#avatar-popup',
   (inputValue) => {
      const promise = new Promise(resolve => {
         ...
         getInfoFromServer.changeAvatar(inputValue['img-link'])
         .then(resolve)
      })
      ...
      .catch(err => console.log(`Ответ аватара не пришел ${err}`))
   }
);
avatarPopup.setEventListeners()

Promise.all([getInfoFromServer.getInfo('users/me'), 
   getInfoFromServer.getInfo('cards')])
...
editProfileButton.addEventListener('click', () => {
   ...
   editProfileInputInfo.value = about;
